...
from .qikt_plus import QIKTPlus
...
def init_model(model_name, model_config, data_config, emb_type, **kwargs):
    if model_name == "dkt":
        ...
        model = QIKT(num_q=data_config['num_q'], num_c=data_config['num_c'],
                max_concepts=data_config['max_concepts'], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"],device=device).to(device) #! 这边 model 不是整体都放到 device 了么
    elif model_name == "qikt+":
        model = QIKTPlus(num_q=data_config['num_q'], num_c=data_config['num_c'],
                max_concepts=data_config['max_concepts'], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"],device=device, **kwargs).to(device) #! 这边 q_pos_neg 通过 kwargs 传进去
