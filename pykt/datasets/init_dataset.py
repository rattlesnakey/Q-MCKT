...
def move_device_collator(batch):
    # keys = list(batch[0].keys())
    # for k in keys:
    collate_batch = defaultdict(list)
    for item in batch:
        for k, v in item.items():
            collate_batch[k].append(v)
    for k, v in collate_batch.items():
        collate_batch[k] = torch.stack(v).to(device)
...
def init_test_datasets(data_config, model_name, batch_size, diff_level=None):
    ...
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=move_device_collator)
    test_window_loader = DataLoader(test_window_dataset, batch_size=batch_size, shuffle=False, collate_fn=move_device_collator)
    if "test_question_file" in data_config:
        print(f"has test_question_file!")

        test_question_loader,test_question_window_loader = None,None
        if not test_question_dataset is None:
            test_question_loader = DataLoader(test_question_dataset, batch_size=batch_size, shuffle=False, collate_fn=move_device_collator)
        if not test_question_window_dataset is None:
            test_question_window_loader = DataLoader(test_question_window_dataset, batch_size=batch_size, shuffle=False, collate_fn=move_device_collator)
...
def init_dataset4train(dataset_name, model_name, data_config, i, batch_size, diff_level=None):
    ...
    if model_name in ["dkt_forget", "bakt_time"]:
        ...
        curvalid = KTQueDataset(os.path.join(data_config["dpath"], data_config["train_valid_file_quelevel"]),
                        ...
                        concept_num=data_config['num_c'], max_concepts=data_config['max_concepts'], data_config=data_config)
    ...
    train_loader = DataLoader(curtrain, batch_size=batch_size, collate_fn=move_device_collator)
    valid_loader = DataLoader(curvalid, batch_size=batch_size, collate_fn=move_device_collator)
    ...
    if model_name in ['qikt+']:
        return train_loader, valid_loader, curtrain.q_pos_neg
    else:
