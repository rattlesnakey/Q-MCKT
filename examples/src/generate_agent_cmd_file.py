...
cancelf = open(sys.argv[8], 'w')

count_run = None
if len(sys.argv) == 10:
    count_run = sys.argv[-1]
...
with open(logf, "r") as fin:
    ...
    while i < len(lines):
        ...
        if idx >= start and idx < end:
            ...
            if count_run:
                cmd = cmd.replace('agent', f'agent --count {count_run}')
            print(cmd)
            outf.write(cmd + "\n")
            cancel_cmd = endcmdpre + sweepid.replace('agent', 'sweep --cancel')
            cancelf.write(cancel_cmd + '\n')
            num += 1
        idx += 1
        i += 4
