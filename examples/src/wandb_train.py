...
def main(params):
    ...
    with open("../../configs/kt_config.json") as f:
        ...
        if model_name in ["qdkt","qikt", "qikt+"] and dataset_name in ['algebra2005','bridge2algebra2006']:
    ...
    if model_name in ['qikt+']:
        train_loader, valid_loader, q_pos_neg = init_dataset4train(dataset_name, model_name, data_config, fold, batch_size)
        kwargs['q_pos_neg'] = q_pos_neg
    ...
    # params_str = "_".join([str(v) for k,v in params.items() if not k in ['other_config', 'save_dir']])
    params_str = "_".join([str(v) for k,v in params.items() if not k in ['other_config', 'save_dir']])
    params_str += "_".join([str(v) for k,v in params['other_config'].items()])
    ...
    model = init_model(model_name, model_config, data_config[dataset_name], emb_type, **kwargs)
    ...
    if model_name == "hawkes":
        ...
        for name, p in model.named_parameters():
            if name in ['model.que_emb.que_emb.weight', 'model.que_emb.concept_emb']:
                cur_dict = {'name':name, 'params': p, 'lr':model.model.contrastive_train_lr}
            else:
                cur_dict = {'name':name, 'params': p, 'lr':learning_rate}
            opt_params.append(cur_dict)
        opt = opt = Adam(opt_params)
    else:
        if optimizer == "sgd":
    ...
    if save_model:
        best_model = init_model(model_name, model_config, data_config[dataset_name], emb_type, **kwargs)
